...
class _LoginFormState extends State<LoginForm> {
  ...
  StreamSubscription<AuthState>? _authSubscription; // ✅ Lắng nghe OAuth callback
  ...
  @override
  void dispose() {
    _authSubscription?.cancel(); // ✅ Hủy lắng nghe khi widget bị destroy
  ...
  // ✅ Đăng nhập Google - CHỈ gọi signInWithOAuth, KHÔNG check session sau đó
  Future<void> _loginGoogle() async {
    try {
      ...
      // Không cần delay hay check session ở đây.
      // onAuthStateChange trong initState sẽ tự xử lý điều hướng
      // sau khi user hoàn thành đăng nhập trên browser.
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
  ...
  // ✅ Đăng nhập Facebook - tương tự Google
