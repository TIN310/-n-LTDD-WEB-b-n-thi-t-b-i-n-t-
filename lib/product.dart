...
class Product {
  ...
  // 👇 ĐÃ SỬA: Toàn bộ key JSON chuyển về chữ thường để khớp với Supabase 👇
  factory Product.fromJson(Map<String, dynamic> json) {
    // 1. Format giá tiền (Sửa 'Gia' thành 'gia')
    int rPrice = (json['gia'] as num?)?.toInt() ?? 0;
    String formattedPrice = rPrice.toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]}.') + ' đ';

    // 2. Xử lý ảnh (Sửa 'MaSP' thành 'masp')
    String img = 'https://picsum.photos/seed/${json['masp']}/500/500';

    // 3. Lấy tên loại sản phẩm (Sửa 'LOAISANPHAM' và 'TenLoai' thành chữ thường)
...
class AppData {
  ...
  // Tính toán tiền ship 30k cứng và trừ thêm Voucher
